...
### Implementation note — pre-lock finalisation, 2026-09-06

No normative content changed. Recorded here because it changes what appears in manifests.

The implementation carried `1.2-dev` for both version counters while the lock was still being prepared. It now carries `1.2`. Development data therefore records `1.1-dev` or `1.2-dev` and formal data records `1.2`; the marker is how the two are told apart at a glance, and no development run is retroactively restamped, because raw evidence is immutable.

Three implementation defects were closed in the same pre-lock change, none of which affects a recorded measurement:

- **Manifest wall-clock span.** `started_at` was a dataclass default that fired when the manifest was constructed — which every caller does after the run has finished — so start and completion recorded the same instant. Across the development dataset 497 of 501 manifests carry a zero-length span, and 4 carry a negative one, where the argument was evaluated a second before the constructor ran. The field is now required and observed where each run begins, and a manifest whose completion precedes its start is refused rather than written. Measurement is unaffected: every RTT comes from monotonic timestamps in the raw records, not from these stamps.
- **Two constants for one frozen parameter.** `frozen.py` declared ten thousand bootstrap replicates while the analysis used two thousand, so the lock would have recorded a number the code never applied. `frozen.py` now owns the value and the analysis imports it. The same consolidation moved the E2 offline count and sync timeline limit, and the E3 sync parameters, into `frozen.py`.
- **Frozen parameters reachable from the environment.** Pilot knobs such as `FAM_E2_TIMELINE_LIMIT` could move a frozen value with no record of it. They remain, because the pilots exist to choose those values, but a run with `publication_data = true` now stops if any is set, or if any effective parameter differs from the lock.

The formal protocol lock of §46 gained an implementation: `results/protocol-lock.json`, generated by `scripts/protocol_lock.py`, validated against its own schema, and checked by every formal run before it starts.

### Implementation note — superseded lock, 2026-09-06

Protocol version 1.2 is unchanged. This records which lock the formal campaign runs under.

The first lock was committed and tagged `protocol-v1.2`. A read-only pre-flight trace of that locked state, run before any collection began, asked a narrow question: for every parameter the lock freezes, which code actually consumes it during E0–E4, and what affects a formal run without appearing in the lock at all. Thirty-one of thirty-one locked parameters matched the values the code would apply, the schedule regenerated from the locked seed reproduced the locked order exactly, and the gate stopped a publication run when a frozen parameter was moved from the environment. Three gaps did not survive the trace.

`environment.rate_limits` was an empty object. §46 requires the rate-limit configuration in the lock; the generator read it from the environment manifest, which never carried that key, while the confirmed values sat in the verifier's own report one file away. The generator now reads them there and refuses to produce a lock without them.

`FAM_E3_SCHEDULE_SEED` was absent from the set of environment variables that stop a publication run. Demonstrated rather than argued: with it set, the gate passed while the first paired block flipped from local-first to federated-first and the lock went on naming seed 20260905. It is now watched, along with `FAM_E3_BOOTSTRAP_SEED`, `FAM_LLM_MAX_TOKENS` and `FAM_LLM_SYSTEM_PROMPT`.

The schedule seed had two definitions — the one the lock reported and the one E3 read — which agreed only by coincidence, exactly as the bootstrap replicate count had. `frozen.py` now owns it.

Two execution parameters that were pinned by the commit alone joined the lock, since a self-contained lock is the point: the agent settle time before each E3 run, now compared like any other frozen parameter, and the complete E4 executor configuration as one hash covering provider, model, base URL, token budget, system prompt and history depth, excluding the API key. E4 refuses to open a session whose effective configuration hashes differently, so §36's requirement that all three sessions use the same frozen configuration is checked against the lock rather than only across the sessions. The E4 session ceilings — how long the runner waits for a person — are recorded but not compared: they bound waiting, not what the tested system does.

The first tag is **not moved**. It produced no evidence: no manifest, raw stream or evidence artifact carries `publication_data = true` under it. It is retained as superseded, and the replacement lock names it in a `supersedes` block, because a published tag that changes what it points at is worse for provenance than a tag recorded as replaced. The formal campaign runs under `protocol-v1.2-lock2`. The protocol version stays 1.2, which is why the replacement is not `v1.2.1`.
